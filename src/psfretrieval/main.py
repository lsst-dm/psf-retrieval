"""The main application factory for the psf-retrieval service.
...
app = FastAPI(
    title="psf-retrieval",
    description=metadata("psf-retrieval")["Summary"],
    version=version("psf-retrieval"),
    openapi_url=f"{config.path_prefix}/openapi.json",
    docs_url=f"{config.path_prefix}/docs",
    redoc_url=f"{config.path_prefix}/redoc",
    lifespan=lifespan,
)
"""The main FastAPI application for psf-retrieval."""
...
if config.slack_webhook:
    logger = structlog.get_logger("psfretrieval")
    SlackRouteErrorHandler.initialize(
        config.slack_webhook, "psf-retrieval", logger
    )
